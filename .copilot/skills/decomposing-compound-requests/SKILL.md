...
2. **Scope each item** — always in parallel, always read-only, one scoper per item. Every scoper reports expected files/areas to touch plus any open question that would block unattended implementation (see "Good checkpoint questions" below — surface ambiguity here, not after dispatch), and must explicitly check shared touchpoints predictions routinely miss: lockfiles, generated files, barrel/route/config registries. Two kinds, chosen per item:
   - **Explore scoper** — read-only Explore agent. For items that repair or mechanically change existing behavior: bug fixes, dependency bumps, renames, one mechanical edit repeated across call sites.
   - **Brainstorm scoper** — read-only agent running superpowers:brainstorming. For any item that would invoke that skill if it arrived alone: creating a feature, building a component, adding functionality, or modifying behavior. Its brief is brainstorming's own process minus the dialogue — explore project context, classify the item spike / bounded / architectural, then work the design out far enough to name the decisions it forces. It returns the four required fields of step 4's brainstormed-item block. **It never asks your human partner anything**: it is isolated and has no channel to them. It returns questions; the checkpoint asks them.
   - Can't tell which kind an item needs → brainstorm scoper. A design pass on a bug fix wastes one dispatch; a missing one puts an unexamined design into an unattended run.
...
4. **Checkpoint — this is a GATE, not a status update.** Present in one message: the work items (with the ask→item mapping and each group's justification), the graph as batches, and every open question batched together (including, when items will land independently: one integration branch/PR, or one per item?).

   **Every brainstorm-scoped item additionally gets its own block carrying all four REQUIRED fields. A block missing a field is an incomplete checkpoint:**
   - **Classification** — spike / bounded / architectural as the scoper classified it *after* exploring, with its one-line reason. This selects the item's route in step 5, so it is your human partner's to override.
   - **Design** — the approach the scoper arrived at, scaled to the classification: a few sentences for bounded; sections covering architecture, components, data flow, error handling and testing for architectural. Questions alone are not a design: your human partner is approving what gets built, not only resolving ambiguities.
   - **Questions** — this item's design questions, merged into the checkpoint's single batched list rather than asked as their own round-trip.
   - **Cut** — what the design deliberately leaves out (YAGNI), one line, so they can pull something back in while the gate is open.

   **This checkpoint IS superpowers:brainstorming's approval gate for every item it scoped** — each design was presented here and answered here. Once they answer, dispatch; never reopen a second per-item approval loop.

   Then **STOP: dispatch no implementer until your human partner has answered** — the resolutions baked into each brief are theirs, not yours. A decision the request already delegated ("if you think it's appropriate") is not a question: state your decision at the checkpoint alongside the questions, so they can override it in the same round-trip. Zero open questions → present the list and graph, state you're proceeding, and continue without waiting. The checkpoint is a dialogue, not a one-shot form: the human may question any item before deciding, and the gate stays closed across the whole exchange until every open question is genuinely answered — a question back is not an answer, and never self-resolve the rest to get moving.
5. **Prepare.** Create an umbrella integration branch in an isolated workspace (superpowers:using-git-worktrees — never implement on main). Write each item's dispatch brief to a file: the item text verbatim, its expected-file list, and the resolved answers. The brief set is the plan that superpowers:subagent-driven-development executes. Brainstorm-scoped items route by the classification your human partner confirmed at the checkpoint:
   - **Architectural** → write the approved design as a spec to `docs/superpowers/specs/YYYY-MM-DD-<topic>-design.md` per superpowers:brainstorming's architectural path, then invoke superpowers:writing-plans against that spec. The resulting `docs/superpowers/plans/YYYY-MM-DD-<topic>.md` **is** that item's dispatch brief — it replaces the one-pager and carries the spec path in its header.
   - **Bounded or spike** → the one-page brief only, with the approved design copied into it verbatim. Brainstorming's bounded path forbids a spec and a plan document; do not write them.
   - Spec and plan are **per work item, never per request**. The umbrella branch, the dependency graph, and the batches are unchanged by them — a planned item is still one work item on one branch, merged in its batch like any other.
...
| Scoping a creative item | Brainstorm scoper: superpowers:brainstorming, read-only, no dialogue. Returns classification + design + questions + cut list for the checkpoint block. Never contacts your human partner. |
...
| Any implementer | Its brief carries its expected-file list (for an architectural item the brief IS its plan document from step 5, dispatched task by task per superpowers:subagent-driven-development); it reports any file touched outside it. Free to invoke skills its own task needs (test-driven-development, systematic-debugging) inside its own context. Design ambiguity the brief doesn't answer → report NEEDS_CONTEXT; never decide alone. |
...
Per item: status (DONE / DONE_WITH_CONCERNS / NEEDS_CONTEXT / BLOCKED, per superpowers:subagent-driven-development's contract), branch and commits, spec and plan paths where written, tests run with results, deviations from the brief, unresolved concerns. Plus an integration section: merge order, each conflict and its resolution, final combined verification result.
...
| "The brainstorm scoper can just ask the user directly" | It is isolated and read-only — it has no channel to them. It returns questions; the checkpoint asks them. One batched gate is the property this skill exists to protect. |
| "It's a feature, so it's architectural" | The scoper classifies after exploring, not from the ask's wording. A new flag on a flow that already exists is bounded: design goes in the brief, no spec, no plan doc. |
| Re-running brainstorming's approval gate per item after the checkpoint | The checkpoint presented every design and they answered. That WAS the gate. Reopening it turns one round-trip back into N and breaks the unattended promise. |
| Giving an obvious bug fix a brainstorm scoper "to be safe" | The tiebreaker applies when you genuinely can't tell. A null-deref fix has no design to settle — Explore scoper. |
...
**Required:** superpowers:subagent-driven-development (implementer/reviewer contracts, sequential mechanics), superpowers:using-git-worktrees (umbrella branch + isolation mechanics), superpowers:brainstorming (scoping, classifying and designing creative items; the spec for architectural ones), superpowers:writing-plans (an architectural item's plan, which becomes its brief), superpowers:finishing-a-development-branch (final integration).
